function [phi_E,phi_P2,phi_Xi]=BGCT_subfun_GPe_Basic(v_xie,v_p2xi,v_p2p2,v_p2d2,v_xip2,v_p1p2,v_ep2)
...
%% --------------new thinking--------------------------------------
%v_ep2 = 1.1e-4;
% v_ep2 = 7.0e-5 * ones(time/dt,1);
% v_ep2 = 1.1e-4*ones(time/dt,1);
% v_xie = 0.1e-3;
% V_xie = zeros(time/dt,1);
v_se = 2.2e-3;
% v_p2xi=4.5e-4;
...
xxs=zeros(1,time/dt);   S_p2MFR=zeros(1,time/dt);   S_xiMFR=zeros(1,time/dt);
...
for t=1:time/dt
    ...
    % v_ep2(t) = 5.0e-5 + (t >= 5/dt && t <= 10/dt)*(t-5/dt)*7.5e-10 + 7.5e-5*(t > 10/dt);
    ...
    % V_xie(t) = 0.05e-3 + (t >= 2/dt && t <= 7/dt)*(t-2/dt)*2.0e-8 + 2.0e-3*(t > 7/dt);
    % v_xie = V_xie(t);
...
% subplot(311),
% plot(dt:dt:time,xxs)
% save('V_xie_timeseries_2015.mat','xxs_Vxie','dt','time','V_xie');
